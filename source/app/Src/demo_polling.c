 ...
 ******************************************************************************
 */
//extern volatile uint8_t txData[PDOUTSIZE],rxData[PDINSIZE];
uint8_t               rxBuf[ 1 + DEMO_NFCV_BLOCK_LEN + RFAL_CRC_LEN ];                        /* Flags + Block Data + CRC *///ԭ��Ϊ�ֲ���������Ϊȫ��
/*
 ******************************************************************************
 * LOCAL VARIABLES
 ******************************************************************************
 ...
 */
#include "App_Api.h"
extern WORD32 NFC_Disable_MsRef;
...
//      static rfalNfcDevice *nfcDevice;  //����ע���ˣ������ں������涨��ȫ�ֱ���(line150)���Է������ߵ��Բ鿴��ֵ
  ...
  case DEMO_ST_DISCOVERY:
    ...
    if( rfalNfcIsDevActivated( rfalNfcGetState() ) )    //��⵽�п�      
    {
      ...
      rfalNfcDeactivate( RFAL_NFC_DEACTIVATE_IDLE );
      if ( API_KnxTm_GetDelayMs( NFC_Disable_MsRef ) >= NFC_DURATION )
      {
          local_info.is_tag_detected = 1;
      }
      local_info.info.tag_type = (NFCTagType)nfcDevice->type+1;
      local_info.info.id_len = nfcDevice->nfcidLen;
//      local_info->tags.info.vipLevel = HIGH;
      memset(local_info.info.id, 0, sizeof(local_info.info.id));
      memcpy(local_info.info.id, nfcDevice->nfcid, nfcDevice->nfcidLen);
...
//    uint8_t               rxBuf[ 1 + DEMO_NFCV_BLOCK_LEN + RFAL_CRC_LEN ];                        /* Flags + Block Data + CRC */
    ...
    uint8_t               wrData[DEMO_NFCV_BLOCK_LEN] = { txData[0], txData[1], txData[2], txData[3]};             /* Write block example */
