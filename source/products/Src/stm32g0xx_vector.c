...
extern BYTE uartreceiveindexclearflag,uartdataupdatflag;
...
void USART2_IRQHandler(void)
{
  ...
  /* Check RTO flag value in ISR register */
  if(LL_USART_IsActiveFlag_RTO(USART2) &&LL_USART_IsEnabledIT_RTO(USART2))
  {
      /* Clear the receiver time out flag*/
      LL_USART_ClearFlag_RTO(USART2);

	  /*receive time out, set the flag to clear the uart recieve index */
	  uartreceiveindexclearflag = 1;

	 /*receive time out, set the flag to check the received data */
	  uartdataupdatflag = 1;

  }
  	 
  if (LL_USART_IsEnabledIT_ERROR(USART2) 
  	  && (LL_USART_IsActiveFlag_NE(USART2)||LL_USART_IsActiveFlag_FE(USART2)))
  {
    /* Call Error function */
   // Error_Callback();
